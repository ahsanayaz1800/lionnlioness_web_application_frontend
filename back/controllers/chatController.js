...
module.exports = {
  ...
  getMessagesCountsById: async (req, res, next) => {
    const user_id = req.params.user_id;
    try {
      const messageCount = await chatModel.getMessagesCountById(user_id); // Updated model function
      return res.status(200).json({ messageCount }); // Return the count instead of the full result
    } catch (error) {
