...
module.exports = {
  ...
  getMessagesCountById: async (user_id) => {
    try {
      const result = await pool.query({
        sql: "SELECT COUNT(*) as messageCount FROM messages WHERE user_id = ?",
        values: [user_id]
      });
      if (result && result[0]) return result[0].messageCount; // Return only the message count
    } catch (err) {
      throw new Error(err);
    }
  },  
